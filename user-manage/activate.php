...
class Activate extends Generic {
	...
	function __construct() {
		...
		if(isset($_SESSION['intranet_clean']['username']))
			$this->user = $_SESSION['intranet_clean']['username'];
	...
	private function getKey() {
		...
		if ($stmt->rowCount() < 1) {
			$this->error = '<div class="alert alert-danger">' . _('Your activation link is incorrect.') . '</div>
					  ...
					  <p>' . sprintf(_('Go to the <a href="http://linux.itvn.fr/back_mini"> homepage</a>'), 'home.php') . '</p>';
			return false;
		}
		...
		if(!empty($_SESSION['intranet_clean']['activate'])) unset($_SESSION['intranet_clean']['activate']);
	...
	private function signedIn() {
		...
		if ($stmt->rowCount() < 1) {
			unset($_SESSION['intranet_clean']['activate']);

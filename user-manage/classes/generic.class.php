...
class Generic extends Connect {
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['intranet_clean']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function guestOnly() {
		...
		if(!empty($_SESSION['intranet_clean']['username'])) {
			$this->error =	"
	...
	public function valid_token($token) {
		...
		if ($_SESSION['intranet_clean']['token'] != $token)
			return false;

...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['intranet_clean']['user_id'] );

		if ( empty($_SESSION['intranet_clean']['user_id']) ) :
			...
			$_SESSION['intranet_clean']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['intranet_clean']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['intranet_clean']['restricted']) || !empty($_SESSION['intranet_clean']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['intranet_clean']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['intranet_clean']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['intranet_clean']['user_id'] ) && $level == array('*') )
			return false;

...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		foreach ($socialLogin as $value) {
			if( !empty($_SESSION['intranet_clean'][$value . 'Misc']) ) {
				$misc = $_SESSION['intranet_clean'][$value . 'Misc'];
			}
		}
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			//$_SESSION['intranet_clean']['username'] = $this->settings[$this->username_type];

			/* Apply default user_level */
			//$_SESSION['intranet_clean']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['intranet_clean']['activate'] = 1;

			//$_SESSION['intranet_clean']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
...
//echo "<pre>";print_r($params);exit;
			...
			//$_SESSION['intranet_clean']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['intranet_clean']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['intranet_clean']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['intranet_clean']['openIDMisc']) ) {
				$link = $_SESSION['intranet_clean']['openIDMisc']['type'];
				$id = $_SESSION['intranet_clean']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['intranet_clean']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['intranet_clean']['twitterMisc']['id'];
			}
			...
			echo "<script>alert('Register success!'); location.href='".$baseURL[0]."overview';</script>";
			//header('Location: ' . parent::getOption('new-user-redirect') );
			exit();
